...
from topotex.data.schema import ConditionerOutput
from topotex.layers.attention import FaceImageAttention
from topotex.layers.topology import (
    TopologyPE,
    TopologyTransformer,
    build_face_graph,
)
from topotex.models.face_tokenizer import FaceTokenizer
from topotex.models.image_encoder import MultiViewEncoder
...
class SurfaceConditioner(nn.Module):
    ...
    def forward(
        self, mesh, mv_images, uv_address, with_rgb=False, face_tokens=None
    ):
        ...
        cond, rgb = self.decoder(
            face_tokens,
            uv_address["face_id"],
            uv_address["barycentric"],
            with_rgb=with_rgb,
        )
        return ConditionerOutput(
            face_tokens=face_tokens,
            uv_condition=cond.unsqueeze(0),
            uv_rgb=rgb.unsqueeze(0) if with_rgb else None,
        )
