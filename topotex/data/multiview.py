...
UNITEX_ROOT = Path(
    os.environ.get("UNITEX_ROOT", "/root/youjiaZhang/UniTEX/upstream")
)
...
class UniTexMV:
    ...
    def _sample_seed(self, sample_id):
        return int.from_bytes(
            hashlib.sha256(f"{self.seed}:{sample_id}".encode()).digest()[:4],
            "little",
        )

    def prepare_inputs(
        self, reference_image, textureless_mesh, sample_id, out_dir
    ):
    ...
    def generate(self, inputs_dir, sample_id, work_dir):
        ...
        alpha = np.asarray(
            Image.open(inputs_dir / "mv_alpha.png").convert("L")
        )
        H, W = alpha.shape
        hp, wp = H // 2, W // 3
        grid_rgb = (
            rgb.reshape(2, hp, 3, wp, 3)
            .transpose(0, 2, 1, 3, 4)
            .reshape(6, hp, wp, 3)
        )
        grid_a = (
            alpha.reshape(2, hp, 3, wp)
            .transpose(0, 2, 1, 3)
            .reshape(6, hp, wp)
            .astype(np.float32)
            / 255.0
