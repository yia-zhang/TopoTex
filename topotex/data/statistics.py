...
python -m topotex.data.statistics [--source output/topotex_source]
...
def main():
    ...
    coverage = sum(
        1
        for r in ds_rows
        if len(r["valid_texels"]) == 4 and min(r["valid_texels"]) > 0
    ) / max(len(ds_rows), 1)
    stats = {
        "attempted_assets": attempted,
        "duplicate_check": {
            "source_duplicates": dup_src,
    ...
    print(
        f"total {len(src_rows)} | failed {len(fails)} "
