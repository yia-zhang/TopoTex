# -*- coding: utf-8 -*-
"""Minimal geometry/rendering utilities (dataset pipeline + evaluation).
...
def camera_matrices(
    ...
    radius = float(
        np.linalg.norm(np.asarray(bbox_max) - np.asarray(bbox_min)) / 2.0
    )
...
def rasterize_view(vertices, faces, cam, res=512):
    ...
    V = torch.as_tensor(
        np.ascontiguousarray(vertices), dtype=torch.float32, device="cuda:0"
    )
    F = torch.as_tensor(
        np.ascontiguousarray(faces), dtype=torch.int32, device="cuda:0"
    )
    mvp = torch.as_tensor(
        cam["proj"] @ cam["view"], dtype=torch.float32, device="cuda:0"
    )
    ...
    return {
        "face_id": fid.cpu().numpy().astype(np.int64),
        "bary": bary.cpu().numpy(),
...
def sample_texture_bilinear(tex_u8, uv):
    """tex [h,w,3] uint8 sRGB; uv [K,2] (v down), REPEAT wrap
    -> linear [K,3]."""
    h, w = tex_u8.shape[:2]
...
def render_albedo_rebake(canon, uvr, atlas_u8, gb):
    ...
    m2u[np.asarray(uvr.uv_face_to_mesh_face, dtype=np.int64)] = np.arange(
        len(uvr.uv_faces)
    )
...
def seam_error(
    ...
    return {
        "seam_error": (
            round(float(np.concatenate(diffs).mean()), 4) if diffs else 0.0
        ),
        "n_seam_edges": n_seam,
