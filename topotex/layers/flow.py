...
class MaskedFlowMatching:
    ...
    def loss(self, model, x0, cond, mask, t=None, generator=None):
        ...
        if t is None:
            t = torch.randint(
                1, self.T + 1, (B,), device=x0.device, generator=generator
            )
        tau = (t.float() / self.T).view(-1, 1, 1, 1)
        eps = torch.randn(
            x0.shape, device=x0.device, generator=generator, dtype=x0.dtype
        )
        xt = (1 - tau) * x0 + tau * eps
    ...
    @torch.no_grad()
    def sample(self, model, cond, mask, steps=50, generator=None, x_init=None):
        ...
        if x_init is not None:
            ...
            x = torch.randn(
                B, 3, H, W, device=mask.device, generator=generator
            )
        x = x * mask
