# -*- coding: utf-8 -*-
"""TOPOTEX evaluation pipeline — closed-loop metrics (Euler-50, shared Z_F).

CLI wrapper: the repository-root ``evaluate.py``.
...
Per mesh:
  ...
  six canonical views each + UV seam consistency on the alternative layout
"""
...
def psnr_region(gt_u8, pred_u8, region):
    mse = float(
        ((gt_u8[region] / 255.0 - pred_u8[region] / 255.0) ** 2).mean()
    )
...
def main():
    ...
    if args.merge:
        ...
        agg = {
            k: round(
                float(np.mean([r[k] for r in rows if r.get(k) is not None])), 2
            )
            for k in num_keys
        }
    ...
    if args.ids_file:
        ...
        pool = (
            blob.get("val") or blob.get("unseen") or blob
    ...
    def sample_query(Z_F, q, mesh):
        ...
        t0 = time.time()
        out = pipe(
            mesh=mesh, uv_query=q, face_tokens=Z_F, num_steps=DDIM, seed=SEED
        )
        torch.cuda.synchronize()
    ...
    def render_q(it, q, tex, vi, res=384):
        ...
        gb = rasterize_view(
            V3, F3, camera_matrices(az, el, V3.min(0), V3.max(0)), res
        )
        ...
        return (
            linear_to_srgb_u8(render_albedo_rebake(canon, uvr, t, gb)),
            gb["mask"],
        )
    ...
    def gt_np(q):
        return (q["gt_texture"].permute(1, 2, 0).cpu().numpy() * 255).astype(
            np.uint8
        )
    ...
    for it in ds.items:
        ...
        gt0 = gt_np(qs["uv_000"])
        r = {
            "mesh_id": it["sample_id"],
            "uv_psnr_canonical": psnr_region(
                gt0, preds["uv_000"], qs["uv_000"]["valid_mask"].cpu().numpy()
            ),
        ...
        s_gen = seam_error(
            F_np, q1["uv_vertices"], q1["uv_faces"], preds["uv_001"], vm1
        )
        s_gt = seam_error(
            F_np, q1["uv_vertices"], q1["uv_faces"], gt_np(q1), vm1
        )
        ...
        json.dumps(
            {
                ...
                "n_meshes": len(rows),
                "protocol": {
                    "generator": ck["config"].get("generator", "fm"),
                    "sampling_steps": DDIM,
                    "seed": SEED,
                    "shared_Z_F": True,
