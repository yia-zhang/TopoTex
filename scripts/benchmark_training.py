...
from topotex.data.dataset import TopoTexDataset  # noqa: E402
from topotex.layers.topology import build_face_graph  # noqa: E402
from topotex.models import MaskedFlowMatching  # noqa: E402
from topotex.models.topotex import build_models  # noqa: E402
...
def step_single(models, diffusion, cfg, it, q, g, autocast):
    ...
    loss = (
        loss.float()
        + 0.1
        * (out["uv_rgb"][0] - q["gt_texture"]).abs()[:, q["valid_mask"]].mean()
    )
...
def group_cond(conditioner, items, packed):
    ...
    for it in items:  # per-mesh views
        ...
        outs.append(
            conditioner.cross(x[fo : fo + n].unsqueeze(0), img).squeeze(0)
        )
        fo += n
    ...
    for s in range(steps):
        ...
        if name in ("A_fp32", "B_bf16"):
            ...
            l = step_single(
                ...
                it["uv_queries"][0],
                g,
                autocast=(name == "B_bf16"),
            )
            n_meshes += 1
        else:
            grp = groups[
                int(torch.randint(0, len(groups), (1,), generator=g_idx))
            ]
            l = step_group(
                models, diffusion, cfg, grp, g, packed=(name == "D_packed")
            )
            n_meshes += len(grp)
...
def main():
    ...
    results = [
        run_variant(v, cfg, ds, args.steps, args.group)
