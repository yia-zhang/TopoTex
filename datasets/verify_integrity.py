...
python -m datasets.verify_integrity --root output/topotex_dataset
...
SOURCE_FILES = ("reference.png", "mv.safetensors", "mesh.safetensors",
...
def check_sample(root, sid, kind, deep, source_root=None):
    ...
    if kind == "source":
        ...
        try:
            ...
            if meta.get("dataset_schema") != SOURCE_SCHEMA:
                fails.append(f"{sid}: dataset_schema "
                             f"{meta.get('dataset_schema')!r}")
        except Exception as e:
            fails.append(f"{sid}: unreadable meta.json ({e})")
        return fails

    for f in ("reference.png", "mv.safetensors", "mesh.safetensors"):
        need(f, _readable_png if f.endswith(".png")
             else _readable_safetensors)
    for q in QUERIES:
        need(f"uv_queries/{q}/uv_address.safetensors",
             _readable_safetensors,
             ("uv_vertices", "uv_faces", "face_id", "barycentric",
              "valid_mask"))
    ...
    if meta.get("query_schema_version") != SCHEMA:
        fails.append(f"{sid}: query_schema_version "
    ...
    if deep and not fails:
        ...
        for q in QUERIES:
            m = qmeta[q]
            arr = load_file(str(d / "uv_queries" / q /
                                "uv_address.safetensors"))
            for key, field in (("face_id", "face_id_sha256"),
        ...
        src_tex = (source_root or (PROJECT_ROOT / "output" /
                                   "topotex_source")) / "samples" / sid \
            / "gt_texture.png"
        if want is None:
...
def run_gate(root, kind="auto", deep=False, limit=None, workers=16,
             source_root=None):
    ...
    if kind == "auto":
        ...
        kind = ("source" if meta.get("dataset_schema") == SOURCE_SCHEMA
                else "dataset")
    failures = [f"manifest: {dup} duplicate ids"] if dup else []
    with ThreadPoolExecutor(max_workers=workers) as ex:
        for fs in ex.map(lambda s: check_sample(root, s, kind, deep,
                                                source_root), ids):
            failures.extend(fs)
    on_disk = {p.name for p in (root / "samples").iterdir() if p.is_dir()}
    extras = sorted(on_disk - set(json.loads(l)["sample_id"]
                                  for l in open(root / "manifest.jsonl")))
    return {"root": str(root), "kind": kind, "deep": deep,
            "n_checked": len(ids), "n_failures": len(failures),
