...
def rasterize_uv(uv_vertices, uv_faces, res, mesh_vertices=None, mesh_faces=None,
                 uv_face_to_mesh_face=None, chart_id_per_face=None,
                 vertex_normals=None, device=None):
    ...
    P = uvv * torch.tensor([W, H], dtype=torch.float64, device=device)  # [Nuv,2]
    tri = P[fcs]                                                        # [F,3,2]
    ...
    if fidx_all.numel() == 0:
        ...
        return AddressMaps(np.zeros((H, W), np.uint8), z - 1, np.zeros((H, W, 3), np.float32),
                           z.copy() - 1, z.copy(), np.zeros((H, W, 3), np.float32),
                           np.zeros((H, W, 3), np.float32), np.zeros((H, W), np.float32),
                           {"num_zero_area": int(zero_area.sum()), "num_flipped": int(flipped.sum())})
    ...
    local = torch.arange(total, device=device) - torch.repeat_interleave(start, counts)
    bwf = bw[face_of_cand]
    lx = local % bwf
    ly = local // bwf
    ...
    # CCW-in-ydown triangles: interior has positive edge fns for edges (0->1,1->2,2->0)
    ...
    def topleft(a, b):
        ...
        # only on the lexicographic sign of the edge direction accepts the pixel
    ...
    bary_n = Es / Es.sum(dim=1, keepdim=True)  # sums to exactly 1 (normalized winding order)
    ...
    if mesh_vertices is not None and mesh_faces is not None:
        ...
        fa3 = torch.cross(V3[F3[:, 1]] - V3[F3[:, 0]], V3[F3[:, 2]] - V3[F3[:, 0]], dim=1).norm(dim=1) * 0.5
...
def verify_address(am, uv_vertices, uv_faces, mesh_vertices, mesh_faces,
                   uv_face_to_mesh_face, res, bary_tol=1e-6, uv_px_tol=0.5,
                   xyz_tol=1e-5, bary_sum_tol=1e-5):
    # bary_sum_tol: 1e-5 fits float32 in-memory maps; pass ~2e-3 when
    # verifying fp16-quantized tensors loaded from disk.
    """Self-checks from the spec; returns dict of measured errors (raises nothing)."""
    ...
    out["face_id_ok"] = bool((fid >= 0).all() and (fid < len(mesh_faces)).all())
    ...
    # xyz reconstruction
    Vc = np.asarray(mesh_vertices, dtype=np.float64)[np.asarray(mesh_faces)[fid]]
    ...
    out["pass"] = bool(out["bary_sum_max_err"] < bary_sum_tol and out["bary_min"] > -bary_tol and
                       out["face_id_ok"] and out["uv_roundtrip_max_px"] <= uv_px_tol and
                       out["xyz_recon_max_err"] < xyz_tol)
    return out
