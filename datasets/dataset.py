...
class TopoTexDataset(torch.utils.data.Dataset):
    def __init__(self, root, sample_ids=None, device="cpu",
                 build_graphs=True):
        from PIL import Image
        from safetensors.numpy import load_file
        from models.surface_conditioner import build_face_graph
        self.root = Path(root)
        ...
        for sid in self.sample_ids:
            ...
            mesh = {"vertices": torch.tensor(mesh_np["vertices"],
                                             device=device),
                    "faces": torch.tensor(mesh_np["faces"].astype(np.int64),
                                          device=device)}
            queries, test_queries = [], []
            for qd in sorted((d / "uv_queries").iterdir()):
                ua = load_file(str(qd / "uv_address.safetensors"))
                gt = np.asarray(Image.open(qd / "gt_texture.png")
                                .convert("RGB"), np.float32) / 255.0
                (test_queries if qd.name.startswith("uv_test")
                 else queries).append({
                    "name": qd.name,
                    "face_id": torch.tensor(ua["face_id"].astype(np.int64),
                                            device=device),
                    "barycentric": torch.tensor(
                        ua["barycentric"].astype(np.float32), device=device),
                    "valid_mask": torch.tensor(
                        ua["valid_mask"].astype(bool), device=device),
                    "gt_texture": torch.tensor(gt.transpose(2, 0, 1),
                                               device=device),
                    "uv_vertices": ua["uv_vertices"],
                    "uv_faces": ua["uv_faces"].astype(np.int64)})
            self.items.append({
                "sample_id": sid,
                "reference_image": torch.tensor(ref.transpose(2, 0, 1),
                                                device=device),
                "mv_images": torch.tensor(mv["images"], device=device),
                "mesh": mesh,
                "graph": (build_face_graph(mesh["vertices"],
                                           mesh["faces"])
                          if build_graphs else None),
                "uv_queries": queries,
