...
python -m datasets.dataset_diversity [--dataset output/topotex_dataset]
...
def _summ(vals, bins=20):
    ...
    return {"min": round(float(v.min()), 4), "p5": round(float(np.percentile(v, 5)), 4),
...
def topology_stats(faces):
    ...
    return {"n_components": len(comp_sizes),
...
def n_islands(uv_faces):
    from .uv_query import face_adjacency  # noqa: F401  (import check)
...
def main():
    ...
    out_path = (Path(args.out) if args.out
                else root / "dataset_diversity.json")
    ...
    res2 = 256 ** 2
    qnames = [("uv_000", "canonical"), ("uv_001", "alternative"),
    ...
    for i, sid in enumerate(ids):
        ...
        ua = load_file(str(d / "uv_queries" / "uv_001" /
        ...
        tex = np.asarray(Image.open(d / "uv_queries" / "uv_000" /
                                    "gt_texture.png").convert("RGB"),
                         np.float32)
        lum = tex.mean(-1)
        ...
        colorful.append(float(np.sqrt(rg.std() ** 2 + yb.std() ** 2)
                              + 0.3 * np.sqrt(rg.mean() ** 2
                                              + yb.mean() ** 2)) / 255)
    ...
    report = {
        "n_samples": len(ids),
        "geometry": {"face_count": _summ(faces),
                     ...
                     "face_degree_hist_0123": [
                         int(x) for x in degree_hist_total],
                     "boundary_edge_ratio": _summ(boundary_ratios)},
        "uv": {"island_count_alternative": _summ(islands),
               "occupancy": {k: _summ(v) for k, v in occ.items() if v}},
        "appearance": {"luminance_mean": _summ(lum_mean),
                       "luminance_std": _summ(lum_std),
                       "colorfulness": _summ(colorful),
                       "luminance_entropy_bits": _summ(entropy)},
    }
    out_path.write_text(json.dumps(report, indent=1))
    print(f"n={len(ids)} | faces p50 {report['geometry']['face_count']['p50']}"
