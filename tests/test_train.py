...
PY = sys.executable
needs_data = pytest.mark.skipif(not (HAVE_DATA and HAVE_CUDA),
                                reason="dataset or GPU unavailable")
...
def test_model_forward_and_gradient_flow():
    ...
    ids = [json.loads(l)["sample_id"]
           for l in open(DATASET / "manifest.jsonl")][:1]
    ...
    for step in range(3):
        ...
        loss = diffusion.loss(dit, x0, out["uv_condition"], mask) \
            + 0.1 * (out["uv_rgb"][0] - q["gt_texture"]).abs()[
                :, q["valid_mask"]].mean()
...
def test_checkpoint_save_load_resume():
    ...
    if run.exists():
        shutil.rmtree(run)
    base = [PY, "train.py", "--config", "configs/topotex_fm_baseline.yaml",
            "--samples", "1", "--run-name", "_test_resume"]
    r1 = subprocess.run(base + ["--steps", "3"], cwd=PROJECT, env=env,
                        capture_output=True, text=True)
    assert r1.returncode == 0, r1.stderr[-800:]
    ck = torch.load(run / "ckpt.pt", map_location="cpu", weights_only=False)
    for k in ("conditioner", "dit", "opt", "rng_g", "rng_g_idx",
              "global_step", "loss_ema"):
        assert k in ck, f"checkpoint missing {k}"
    assert ck["global_step"] == 3
    r2 = subprocess.run(base + ["--steps", "6", "--resume"], cwd=PROJECT,
