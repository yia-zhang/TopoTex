...
def test_model_import_and_forward():
    ...
    with torch.no_grad():
        out = model({"vertices": V, "faces": F}, torch.rand(1, 6, 3, 64, 64),
...
@pytest.mark.skipif(not (HAVE_DATA and HAVE_CKPT and HAVE_CUDA),
...
def test_one_sample_conditioner_forward():
    ...
    ck = torch.load(PROJECT / "checkpoints" / "baseline" / "ckpt.pt",
