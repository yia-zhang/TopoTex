...
from datasets.verify_integrity import run_gate, sha256_bytes
...
def make_dataset(root, n=2):
    ...
    for sid in ids:
        ...
        for q in QUERIES:
            ...
            save_file({"uv_vertices": np.zeros((3, 2), np.float32),
            ...
            Image.fromarray(np.zeros((8, 8, 3), np.uint8)).save(
                qd / "gt_texture.png")
            queries.append({"name": q, "type": "canonical",
                            "held_out": q == "uv_test", "valid_texels": 0,
                            "face_id_sha256": sha256_bytes(fid.tobytes()),
                            "barycentric_sha256":
                                sha256_bytes(bary.tobytes())})
        (d / "meta.json").write_text(json.dumps(
            {"sample_id": sid, "schema": SCHEMA, "num_faces": 1,
             "uv_queries": queries, "query_schema_version": SCHEMA,
...
def test_unreadable_required_file_fails(tmp_path):
    ds, src, ids = make_dataset(tmp_path)
    p = ds / "samples" / ids[1] / "uv_queries" / "uv_000" / \
...
def test_deep_hash_drift_fails(tmp_path):
    ...
    ds, src, ids = make_dataset(tmp_path)
    p = ds / "samples" / ids[0] / "uv_queries" / "uv_002" / \
        "uv_address.safetensors"
...
@pytest.mark.skipif(
    not (Path(__file__).resolve().parents[1] / "output" / "topotex_dataset"
