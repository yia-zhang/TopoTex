...
from datasets.build_dataset import (SAMPLE_FILES, SkipSample,
                                   check_sample_schema, extract_glb,
                                   sample_is_valid, uv_address)
...
def _make_glb(tmp_path, two_textures=False, overlap=False):
    ...
    if overlap:
        uv = np.array([[.1, .1], [.9, .1], [.9, .9], [.8, .2]])
    else:
        uv = np.array([[.1, .1], [.9, .1], [.9, .9], [.1, .9]])
    tex = Image.fromarray((np.random.default_rng(0).integers(
        0, 255, (64, 64, 3))).astype(np.uint8))
    mat = trimesh.visual.material.PBRMaterial(baseColorTexture=tex)
    m = trimesh.Trimesh(v, f, visual=trimesh.visual.TextureVisuals(
        uv=uv, material=mat), process=False)
    if two_textures:
        tex2 = Image.fromarray(np.full((32, 32, 3), 200, np.uint8))
        mat2 = trimesh.visual.material.PBRMaterial(baseColorTexture=tex2)
        m2 = trimesh.Trimesh(v + 2.0, f, visual=trimesh.visual.TextureVisuals(
            uv=uv, material=mat2), process=False)
