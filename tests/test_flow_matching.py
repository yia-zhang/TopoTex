...
def test_loss_ignores_invalid_region():
    ...
    l0 = fm.loss(
        Base(0.0), x0, None, m, t=torch.tensor([300, 700]), generator=g
    )
    g = torch.Generator().manual_seed(0)
    l1 = fm.loss(
        Base(123.0), x0, None, m, t=torch.tensor([300, 700]), generator=g
    )
...
def test_euler_keeps_invalid_fixed_and_range():
    ...
    x = fm.ddim_sample(
        Zero(), None, m, steps=10, generator=torch.Generator().manual_seed(0)
    )
...
def test_euler_recovers_memorized_x0():
    ...
    x = fm.ddim_sample(
        Oracle(), None, m, steps=50, generator=torch.Generator().manual_seed(1)
    )
