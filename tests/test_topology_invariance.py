...
def coincident_fixture():
    ...
    V = torch.tensor(
        tri
        + [[1.3, 1.1, 0.0], [2.1, 0.2, 0.0]]  # strip extras
        + tri,
        dtype=torch.float32,
    )  # coincident copy
    F = torch.tensor(
        [
            [0, 1, 2],  # face A (part of strip)
            [1, 3, 2],  # strip
            [1, 4, 3],  # strip
            [5, 6, 7],
        ]
    )  # face B == face A in XYZ, isolated
...
def test_coincident_face_rgb_fit_vs_xyz_baseline():
    ...
    for _ in range(150):
        ...
        opt.step()
    assert loss < 0.05, (
        f"face-token path failed to fit red/blue: {float(loss)}"
    )
    ...
    xyz = (
        V[F[uv["face_id"][valid]].long()]
        * uv["barycentric"][valid].unsqueeze(-1)
    ).sum(1)
    ...
    for _ in range(300):
        ...
        opt2.step()
    assert l2 > 0.25, (
        f"xyz baseline beat its provable 1/3 floor?!: {float(l2)}"
    )
